...
Run Principal Component Analysis (PCA) on raw hyperspectral data columns
(`nm940` – `nm1720`) and visualise the first three components in a 3‑D
scatter plot. The script is geared towards classifying plastics, so it
uses the **PlasticType** column as the default label, but you can override
that with `-t/--target`.
...
Example
-------
python pca_plot.py -f combined_DB22_measurements_sorted_clean.csv -c 6
"""
