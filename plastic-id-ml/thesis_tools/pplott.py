"""
Mean absorbance (0‒0.8) per PlasticType
=================================================================
This script plots the mean absorbance of each plastic type across
the wavelengths in the dataset. It takes the CSV file as input,
processes the data to calculate absorbance, and generates a plot with the following features:
...
wavelengths = np.array([int(c[2:]) for c in raw_cols])

# -------------------------------------------------- reflectance -> absorbance (0–1) then mean
R_max = df[raw_cols].max().replace(0, np.nan)
absorbance_df = 1 - df[raw_cols].divide(R_max)

mean_abs = absorbance_df.join(df["PlasticType"]).groupby("PlasticType")[raw_cols].mean()
...
handles = [h[l.index(k)] for k in order]
labels = order
